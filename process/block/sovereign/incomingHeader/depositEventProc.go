...
func (dep *depositEventProc) ProcessEvent(event data.EventHandler) (*EventResult, error) {
	...
	scr := &smartContractResult.SmartContractResult{
		...
		OriginalTxHash: nil, // TODO:  Implement this in DRT-14321 task
		RcvAddr:        topics[1],
...
func (dep *depositEventProc) createSCRData(topics [][]byte) ([]byte, error) {
	...
	ret := []byte(core.BuiltInFunctionMultiDCDTNFTTransfer +
...
func (dep *depositEventProc) getTokenDataBytes(tokenNonce []byte, tokenData []byte) ([]byte, error) {
	dcdtTokenData, err := dep.dataCodec.DeserializeTokenData(tokenData)
	if err != nil {
		return nil, err
	}

	if dcdtTokenData.TokenType == core.Fungible {
		return dcdtTokenData.Amount.Bytes(), nil
	}
	...
	digitalToken := &dcdt.DCDigitalToken{
		Type:  uint32(dcdtTokenData.TokenType),
		Value: dcdtTokenData.Amount,
		TokenMetaData: &dcdt.MetaData{
			Nonce:      nonce,
			Name:       dcdtTokenData.Name,
			Creator:    dcdtTokenData.Creator,
			Royalties:  uint32(dcdtTokenData.Royalties.Uint64()),
			Hash:       dcdtTokenData.Hash,
			URIs:       dcdtTokenData.Uris,
			Attributes: dcdtTokenData.Attributes,
		},
	}

...
func TestNewSovereignAPIRewardsHandler(t *testing.T) {
	t.Parallel()

	t.Run("nil input", func(t *testing.T) {
		apiRewardsHandler, err := NewSovereignAPIRewardsHandler(nil)
		require.Nil(t, apiRewardsHandler)
		require.Equal(t, errors.ErrNilAddressPublicKeyConverter, err)
	})

	t.Run("should work", func(t *testing.T) {
		apiRewardsHandler, err := NewSovereignAPIRewardsHandler(&testscommon.PubkeyConverterMock{})
		require.Nil(t, err)
		require.False(t, apiRewardsHandler.IsInterfaceNil())
	})
}

func TestSovereignAPIRewardsHandlerPrepareRewardTx(t *testing.T) {
	t.Parallel()
	...
	apiRewardsHandler, _ := NewSovereignAPIRewardsHandler(pkConv)

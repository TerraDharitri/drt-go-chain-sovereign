...
import (
	...
	factoryErrors "github.com/TerraDharitri/drt-go-chain/factory"
...
func TestCreateApiResolver(t *testing.T) {
	...
	t.Run("NewDCDTTransferParser fails causing createScQueryElement error", func(t *testing.T) {
		failingStepsInstance.reset()
	...
	t.Run("APIRewardsTxHandlerField nil should error", func(t *testing.T) {
		...
		args.RunTypeComponents = runTypeCompsStub
		apiResolver, err := api.CreateApiResolver(args)
		require.Equal(t, errDrt.ErrNilAPIRewardsHandler, err)
		require.True(t, check.IfNil(apiResolver))
	})
}
	...
	return api.SCQueryElementArgs{
		GeneralConfig: &config.Config{
			...
			BuiltInFunctions: config.BuiltInFunctionsConfig{
				...
				DNSV2Addresses:                []string{"drt1qqqqqqqqqqqqqpgqr46jrxr6r2unaqh75ugd308dwx5vgnhwh47qkswz60"},
			},

...
const (
	...
	simpleDcdtSafeWasmPath = "testdata/simple-dcdt-safe.wasm"
...
// - whitelist the bridge dcdt safe contract to allow it to burn/mint cross chain dcdt tokens
...
func testExecuteOperationNotAllowedToMintTokenWithoutPrefix(
	...
	bridgeData := deployBridgeSetup(t, cs, initialAddress, argsDcdtSafe, enshrineDcdtSafeContract, simpleEnshrineDcdtSafeWasmPath)
...
// Test flow:
// - deploy sovereign bridge contracts on the main chain
// - whitelist WRONG bridge dcdt safe contract not allowing to burn/mint cross chain dcdt tokens
...
func testExecuteOperationNotAllowedToMintFungibleContractNotWhitelisted(
	...
	whiteListedAddress := "drt1qqqqqqqqqqqqqpgqcw92wj0huvaghg4aeuykknp7hstmrmhudz3s2w65g4"
	cs, err := chainSimulator.NewChainSimulator(chainSimulator.ArgsChainSimulator{
		BypassTxSignatureCheck:   true,
		TempDir:                  t.TempDir(),
		PathToInitialConfig:      defaultPathToInitialConfig,
	...
	// dcdt-safe address generated is NOT whitelisted
	dcdtSafeEncoded, _ := nodeHandler.GetCoreComponents().AddressPubKeyConverter().Encode(bridgeData.DCDTSafeAddress)
	require.NotEqual(t, whiteListedAddress, dcdtSafeEncoded)

	wallet, err := cs.GenerateAndMintWalletAddress(0, chainSim.InitialAmount)
	require.Nil(t, err)

	// execute operation, bridge in token with prefix
	// expecting not allowed to mint because DCDT-safe contract is not whitelisted
	txResult := executeOperation(t, cs, bridgeData.OwnerAccount.Wallet, wallet.Bytes, &bridgeData.OwnerAccount.Nonce, bridgeData.DCDTSafeAddress, bridgedInTokens, wallet.Bytes, nil)
	chainSim.RequireSignalError(t, txResult, actionNotAllowed)
}
...
func testDepositNotAllowedToBurnTokensContractNotWhitelisted(
	...
	walletAddress := "drt1j7t00tq4jhcarcl2kuw3aywplnqlh9y6w9sxx2v6namc3jxvn5qqjwtcqf"
	...
	for _, token := range bridgedOutTokens {
		...
		chainSim.SetDcdtInWallet(t, cs, wallet, token.Identifier, token.Nonce, tokenData)
	...
	// expecting these tokens to NOT be burned by DCDT safe sc because is not whitelisted
	txResult := deposit(t, cs, wallet.Bytes, &nonce, bridgeData.DCDTSafeAddress, bridgedOutTokens, wallet.Bytes)
	chainSim.RequireSignalError(t, txResult, actionNotAllowed)
}

func getTokenMetaData(token chainSim.ArgsDepositToken) *dcdt.MetaData {
...
func TestChainSimulator_ExecuteSameNft2Times(t *testing.T) {
	...
	nftToken := chainSim.ArgsDepositToken{
		Identifier: argsDcdtSafe.ChainPrefix + "-NFTV2-a1b2c3",
		Nonce:      uint64(7),
		Amount:     big.NewInt(1),
	...
	for _, bridgedInToken := range groupTokens(bridgedInTokens) {
		checkMetaDataInAccounts(t, cs, bridgedInToken, dcdtSafeEncoded, big.NewInt(1))

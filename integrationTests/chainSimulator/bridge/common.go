...
// ArgsDcdtSafe holds the arguments for dcdt safe contract argument
type ArgsDcdtSafe struct {
	ChainPrefix       string
...
func initOwnerAndSysAccState(
	...
	tokenKey := hex.EncodeToString([]byte(core.ProtectedKeyPrefix + core.DCDTKeyIdentifier + argsDcdtSafe.IssuePaymentToken))
...
func deployBridgeSetup(
	...
	ownerAddress string,
	argsDcdtSafe ArgsDcdtSafe,
	deployDcdtSafeContract dcdtSafeDeployFunc,
	contractWasmPath string,
) *ArgsBridgeSetup {
	...
	dcdtSafeAddress := deployDcdtSafeContract(t, cs, ownerAddrBytes, &nonce, systemContractDeploy, argsDcdtSafe, contractWasmPath)
	...
	chainSim.SendTransactionWithSuccess(t, cs, ownerAddrBytes, &nonce, dcdtSafeAddress, chainSim.ZeroValue, setFeeMarketAddressData, uint64(10000000))

	chainSim.SendTransactionWithSuccess(t, cs, ownerAddrBytes, &nonce, dcdtSafeAddress, chainSim.ZeroValue, "unpause", uint64(10000000))
...
func dcdtSafeContract(
	...
	dcdtSafeArgs := "@00000000000000000500f3c3c6f64f8a20ced95a45a7de596ab08f9082d0f8ef" // dummy header_verifier_address
	return chainSim.DeployContract(t, cs, ownerAddress, nonce, systemContractDeploy, dcdtSafeArgs, contractWasmPath)
}

func enshrineDcdtSafeContract(
	t *testing.T,
	cs chainSim.ChainSimulator,
	ownerAddress []byte,
	nonce *uint64,
	...
	dcdtSafeArgs := "@" + // is_sovereign_chain
		"@01" + lengthOn4Bytes(len(argsDcdtSafe.IssuePaymentToken)) + hex.EncodeToString([]byte(argsDcdtSafe.IssuePaymentToken)) + // token identifier
		"@01" + lengthOn4Bytes(len(argsDcdtSafe.ChainPrefix)) + hex.EncodeToString([]byte(argsDcdtSafe.ChainPrefix)) // prefix
	return chainSim.DeployContract(t, cs, ownerAddress, nonce, systemContractDeploy, dcdtSafeArgs, contractWasmPath)
}
...
func registerSovereignNewTokens(
	...
	registerData := core.BuiltInFunctionDCDTTransfer +
	...
	txResult := chainSim.SendTransaction(t, cs, wallet.Bytes, nonce, dcdtSafeAddress, chainSim.ZeroValue, registerData, uint64(10000000))
...
func executeOperation(
	...
	return chainSim.SendTransaction(t, cs, wallet.Bytes, nonce, dcdtSafeAddress, chainSim.ZeroValue, executeBridgeOpsData, uint64(100000000))
}
...
func getTokenDataArgs(creator []byte, tokens []chainSim.ArgsDepositToken) string {
	...
	for _, token := range tokens {
		arg = arg +
			...
			hex.EncodeToString([]byte("DCDT")) + // name

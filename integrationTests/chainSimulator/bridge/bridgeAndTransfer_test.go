...
// TODO: DRT-15527 Make a similar bridge test with sovereign chain simulator after merging this into feat/chain-go-sdk

// In this test we:
// - will deploy sovereign bridge contracts on the main chain
// - will whitelist the bridge dcdt safe contract to allow it to burn/mint cross chain dcdt tokens
...
func TestChainSimulator_ExecuteMintBurnBridgeOpForDCDTTokensWithPrefixAndTransfer(t *testing.T) {
	if testing.Short() {
	...
	// TODO DRT-15942 uncomment after rust framework and contract framework will be updated with all dcdt types
	bridgedInTokens := make([]chainSim.ArgsDepositToken, 0)
	bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
		Identifier: argsDcdtSafe.ChainPrefix + "-TKN-123456",
		Nonce:      uint64(0),
		Amount:     big.NewInt(555666),
		Type:       core.Fungible,
	})
	bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
		Identifier: argsDcdtSafe.ChainPrefix + "-NFTV2-1a2b3c",
		Nonce:      uint64(3),
		Amount:     big.NewInt(1),
		Type:       core.NonFungibleV2,
	})
	//bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
	//	Identifier: argsDcdtSafe.ChainPrefix + "-DNFT-ead43f",
	//	Nonce:      uint64(6),
	//	Amount:     big.NewInt(1),
	//	Type:       core.DynamicNFT,
	//})
	bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
		Identifier: argsDcdtSafe.ChainPrefix + "-SFT-cedd55",
		Nonce:      uint64(52),
		Amount:     big.NewInt(2156),
		Type:       core.SemiFungible,
	})
	//bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
	//	Identifier: argsDcdtSafe.ChainPrefix + "-DSFT-f6b4c2",
	//	Nonce:      uint64(33),
	//	Amount:     big.NewInt(11),
	//	Type:       core.DynamicSFT,
	//})
	//bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
	//	Identifier: argsDcdtSafe.ChainPrefix + "-META-4b543b",
	//	Nonce:      uint64(57),
	//	Amount:     big.NewInt(124000),
	//	Type:       core.MetaFungible,
	//})
	//bridgedInTokens = append(bridgedInTokens, chainSim.ArgsDepositToken{
	//	Identifier: argsDcdtSafe.ChainPrefix + "-DMETA-4b543b",
	//	Nonce:      uint64(31),
	...
	// expecting these tokens to be minted by the whitelisted DCDT safe sc and transferred to our address from another shard.
	...
	for _, bridgedInToken := range groupTokens(bridgedInTokens) {
		checkTokenDataInAccount(t, cs, bridgedInToken, dcdtSafeEncoded, big.NewInt(0))       // sender
	...
	for _, token := range groupTokens(bridgedInTokens) {
		...
		txResult = deposit(t, cs, receiver.Bytes, &receiverNonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{bridgedOutToken}, receiver.Bytes)
		chainSim.RequireSuccessfulTransaction(t, txResult)
		...
		checkTokenDataInAccount(t, cs, token, dcdtSafeEncoded, big.NewInt(0))
...
func transferToken(
	...
	if token.Type == core.Fungible {
		chainSim.TransferDCDT(t, cs, sender.Bytes, receiver.Bytes, senderNonce, token.Identifier, amountToSend)
	} else {
		chainSim.TransferDCDTNFT(t, cs, sender.Bytes, receiver.Bytes, senderNonce, token.Identifier, token.Nonce, amountToSend)
		err := cs.GenerateBlocks(3)
		require.Nil(t, err)
	}

...
// TODO DRT-15286 split data codec in multiple components
...
// SerializeTokenData will receive an dcdt token data and serialize it
func (dc *dataCodec) SerializeTokenData(tokenData sovereign.DcdtTokenData) ([]byte, error) {
...
func getTokenDataStruct(tokenData sovereign.DcdtTokenData) *abi.StructValue {
...
// DeserializeTokenData will deserialize bytes to an dcdt token data
func (dc *dataCodec) DeserializeTokenData(data []byte) (*sovereign.DcdtTokenData, error) {
	if len(data) == 0 {
	...
	return &sovereign.DcdtTokenData{
		TokenType:  core.DCDTType(enum.Discriminant),
		Amount:     amount.Value,
		Frozen:     frozen.Value,
		Hash:       hash.Value,
		Name:       name.Value,
...
func getOperationTokens(tokens []sovereign.DcdtToken) []abi.SingleValue {
...
func getTokenStruct(token sovereign.DcdtToken) *abi.StructValue {

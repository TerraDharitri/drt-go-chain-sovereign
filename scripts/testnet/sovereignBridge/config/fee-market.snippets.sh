FEE_MARKET_ADDRESS=$(drtpy data load --use-global --partition=${CHAIN_ID} --key=address-fee-market-contract)
FEE_MARKET_ADDRESS_SOVEREIGN=$(drtpy data load --use-global --partition=sovereign --key=address-fee-market-contract)
...
deployFeeMarketContract() {
    ...
    drtpy data store --use-global --partition=${CHAIN_ID} --key=address-fee-market-contract --value=${ADDRESS}
    FEE_MARKET_ADDRESS=$(drtpy data load --use-global --partition=${CHAIN_ID} --key=address-fee-market-contract)
    echo -e "Fee Market contract: ${ADDRESS}"

    local SOVEREIGN_CONTRACT_ADDRESS=$(computeSecondSovereignContractAddress)
    drtpy data store --use-global --partition=sovereign --key=address-fee-market-contract --value=${SOVEREIGN_CONTRACT_ADDRESS}
    FEE_MARKET_ADDRESS_SOVEREIGN=$(drtpy data load --use-global --partition=sovereign --key=address-fee-market-contract)
...
distributeFees() {
    ...
    drtpy contract call ${FEE_MARKET_ADDRESS} \
        --pem=${WALLET} \
        --proxy=${PROXY} \
        --chain=${CHAIN_ID} \
        --gas-limit=20000000 \

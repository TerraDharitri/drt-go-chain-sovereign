...
copyConfig() {
  ...
  if [ "$SOVEREIGN_DEPLOY" -eq 1 ]; then
      cp "$DHARITRIDIR"/../drt-go-chain-sovereign-bridge/cert/cmd/cert/private_key.pem ./node/config
      cp "$DHARITRIDIR"/../drt-go-chain-sovereign-bridge/cert/cmd/cert/certificate.crt ./node/config
  fi
...
updateElasticsearchIndices() {
  ...
  pushd $(dirname $DHARITRIDIR)/drt-go-chain-tools/elasticreindexer/cmd/indices-creator
  ./indices-creator
  popd
}

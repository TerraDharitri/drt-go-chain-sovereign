...
func TestSovereignChainSimulator_NewDelegationSC(t *testing.T) {
	...
	wallet, err := cs.GenerateAndMintWalletAddress(core.SovereignChainShardId, big.NewInt(0).Mul(chainSim.OneREWA, big.NewInt(2500)))
	require.Nil(t, err)
	nonce := uint64(0)
	...
	txData := "createNewDelegationContract" +
		"@" + hex.EncodeToString(big.NewInt(0).Mul(chainSim.OneREWA, big.NewInt(100000)).Bytes()) +
		"@64"
	cost := big.NewInt(0).Mul(chainSim.OneREWA, big.NewInt(1250))

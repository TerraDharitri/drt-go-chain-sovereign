...
func (sc *sovereignSCProcessor) ProcessSmartContractResult(scr *smartContractResult.SmartContractResult) (vmcommon.ReturnCode, error) {
	...
	if !bytes.Equal(scr.SndAddr, core.DCDTSCAddress) {
		return returnCode, fmt.Errorf("%w, expected DCDTSCAddress", errInvalidSenderAddress)
	}
...
func (sc *sovereignSCProcessor) checkBuiltInFuncCall(scrData string) error {
	...
	if function != core.BuiltInFunctionMultiDCDTNFTTransfer {
		return fmt.Errorf("%w, expected %s", errInvalidBuiltInFunctionCall, core.BuiltInFunctionMultiDCDTNFTTransfer)
	}

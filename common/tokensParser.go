...
func ExtractTokenIDAndNonceFromTokenStorageKey(tokenKey []byte) ([]byte, bool, uint64) {
	...
	nonceStr := tknData.randomSequencePlusNonce[dcdtTickerNumRandChars:]
...
func isTokenDataValid(tknData *tokenData) bool {
	...
	isTickerValid := dcdt.IsTickerValid(tknData.ticker) && dcdt.IsTokenTickerLenCorrect(tokenTickerLen)
	isRandomSequencePlusNonceValid := len(tknData.randomSequencePlusNonce) >= dcdtTickerNumRandChars+1
...
func isValidPrefixedToken(token string) bool {
	...
	prefix := tokenSplit[0]
	if !dcdt.IsValidTokenPrefix(prefix) {
		return false
	}

	tokenTicker := tokenSplit[1]
	if !dcdt.IsTickerValid(tokenTicker) {
		return false
	}

	tokenRandSeq := tokenSplit[2]
	return len(tokenRandSeq) >= dcdtTickerNumRandChars
}

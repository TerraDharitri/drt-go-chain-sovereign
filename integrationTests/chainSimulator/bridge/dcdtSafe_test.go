...
type rustDCDTType uint32

const (
	fungible rustDCDTType = iota
	nonFungible
	semiFungible
	metaFungible
	dynamicNFT
...
// dcdt-safe contract in main chain will issue its own tokens at registerToken step
...
func TestChainSimulator_ExecuteAndDepositTokensWithPrefix(t *testing.T) {
	...
	cs, err := chainSimulator.NewChainSimulator(chainSimulator.ArgsChainSimulator{
		...
		AlterConfigsFunction: func(cfg *config.Configs) {
			cfg.SystemSCConfig.DCDTSystemSCConfig.BaseIssuingCost = issuePaymentCost
		},
	})
	require.Nil(t, err)
	require.NotNil(t, cs)
	...
	bridgeData := deployBridgeSetup(t, cs, initialAddress, ArgsDcdtSafe{}, dcdtSafeContract, dcdtSafeWasmPath)
	dcdtSafeAddr, _ := cs.GetNodeHandler(0).GetCoreComponents().AddressPubKeyConverter().Encode(bridgeData.DCDTSafeAddress)
	dcdtSafeAddrShard := chainSim.GetShardForAddress(cs, dcdtSafeAddr)
	...
	// TODO DRT-15942 uncomment sft/meta when DCDTTYpe VM hook will be implemented
	// because get_dcdt_token_data doesn't return real type and deposit will clear the storage
	...
	for _, token := range tokens {
		...
		registerTokens(t, cs, wallet, &nonce, bridgeData.DCDTSafeAddress, token)
		tokensMapper[token.Identifier] = chainSim.GetIssuedDcdtIdentifier(t, cs, getTokenTicker(token.Identifier), "")
		...
		// expecting the token to be minted in dcdt-safe contract with the same properties and transferred to receiver address
		// -------------
		// for (dynamic) SFT/MetaDCDT the contract will create one more token and keep it forever
		// because if the same token is received 2nd time, the contract will just add quantity, not create different token
		txResult := executeOperation(t, cs, bridgeData.OwnerAccount.Wallet, receiver.Bytes, &bridgeData.OwnerAccount.Nonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{token}, wallet.Bytes, nil)
		...
		if isSftOrMeta(receivedToken.Type) {
			chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(receivedToken), dcdtSafeAddr, big.NewInt(1))
		} else {
			chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(receivedToken), dcdtSafeAddr, big.NewInt(0))
		}

		waitIfCrossShardProcessing(cs, dcdtSafeAddrShard, receiverShardId)
		...
		txResult = deposit(t, cs, receiver.Bytes, &receiverNonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{depositToken}, wallet.Bytes)
		chainSim.RequireSuccessfulTransaction(t, txResult)
		...
		if isSftOrMeta(receivedToken.Type) {
			// for (dynamic) SFT/MetaDCDT, the contract should always have 1 dcdt
			expectedAmount = big.NewInt(1)
		}
		chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(depositToken), dcdtSafeAddr, expectedAmount)

		_ = cs.GenerateBlocks(1)
		tokenSupply, err := cs.GetNodeHandler(dcdtSafeAddrShard).GetFacadeHandler().GetTokenSupply(getTokenIdentifier(depositToken))
		require.Nil(t, err)
	...
	// for (dynamic) SFT/MetaDCDT, the contract should just add quantity for existing nonce
	for _, token := range tokens {
		// get contract from next shard
		receiver := receiverContracts[receiverShardId]

		// execute operations received from sovereign chain
		// expecting the token to be minted in dcdt-safe contract with the same properties and transferred to receiver contract address
		trnsData := &transferData{
			GasLimit: uint64(10000000),
			Function: []byte("hello"),
			Args:     [][]byte{{0x01}},
		}
		txResult := executeOperation(t, cs, bridgeData.OwnerAccount.Wallet, receiver.Bytes, &bridgeData.OwnerAccount.Nonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{token}, wallet.Bytes, trnsData)
		chainSim.RequireSuccessfulTransaction(t, txResult)
		receivedToken := chainSim.ArgsDepositToken{
			Identifier: tokensMapper[token.Identifier],
			Nonce:      token.Nonce,
		...
		waitIfCrossShardProcessing(cs, dcdtSafeAddrShard, receiverShardId)
		_ = cs.GenerateBlocks(1) // one more block required for DCDTTransfer dcdt-safe -> hello
...
// dcdt-safe contract in main chain will save the tokens at deposit, then send from balance at executeOperation
func TestChainSimulator_DepositAndExecuteOperations(t *testing.T) {
	...
	// expecting that tokens to be saved in dcdt-safe contract after deposit
	for account, token := range wallets {
		...
		txResult := deposit(t, cs, accountAddrBytes, &account.nonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{depositToken}, accountAddrBytes)
		chainSim.RequireSuccessfulTransaction(t, txResult)
		...
		waitIfCrossShardProcessing(cs, chainSim.GetShardForAddress(cs, account.addrBech32), dcdtSafeAddrShard)
		chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(depositToken), dcdtSafeAddr, amountToTransfer)
	...
	for account, token := range wallets {
		...
		// execute operations received from sovereign chain
		// expecting the token to be transferred from dcdt-safe contract to wallet address
		txResult := executeOperation(t, cs, bridgeData.OwnerAccount.Wallet, accountAddrBytes, &bridgeData.OwnerAccount.Nonce, bridgeData.DCDTSafeAddress, []chainSim.ArgsDepositToken{executeToken}, accountAddrBytes, nil)
		chainSim.RequireSuccessfulTransaction(t, txResult)
		chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(executeToken), dcdtSafeAddr, big.NewInt(0))
		waitIfCrossShardProcessing(cs, dcdtSafeAddrShard, chainSim.GetShardForAddress(cs, account.addrBech32))
...
func TestChainSimulator_ExecuteWithTransferDataFails(t *testing.T) {
	...
	for _, token := range tokens {
		...
		// execute operations received from sovereign chain
		// expecting the token to be minted in dcdt-safe contract with the same properties and transferred with SC call to hello contract
		// for (dynamic) SFT/MetaDCDT the contract will create one more token and keep it forever
		trnsData := &transferData{
			GasLimit: uint64(10000000),
			Function: []byte("hello"),
		...
		waitIfCrossShardProcessing(cs, dcdtSafeAddrShard, receiverShardId)
		// still no tokens in dcdt-safe, tokens should be burned
		if isSftOrMeta(receivedToken.Type) {
			chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(receivedToken), dcdtSafeAddr, big.NewInt(1))
		...
		tokenSupply, err := cs.GetNodeHandler(dcdtSafeAddrShard).GetFacadeHandler().GetTokenSupply(getTokenIdentifier(receivedToken))
		require.Nil(t, err)
		require.NotNil(t, tokenSupply)
...
func generateAccountsAndTokens(
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId := chainSim.RegisterAndSetAllRoles(t, cs, accountAddrBytes, &account.nonce, issueCost, "NFTV2", "NFTV2", core.NonFungibleDCDTv2, 0)
	supply = big.NewInt(1)
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId = chainSim.RegisterAndSetAllRolesDynamic(t, cs, accountAddrBytes, &account.nonce, issueCost, "DNFT", "DNFT", core.DynamicNFTDCDT, 0)
	supply = big.NewInt(1)
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId = chainSim.RegisterAndSetAllRoles(t, cs, accountAddrBytes, &account.nonce, issueCost, "SFT", "SFT", core.SemiFungibleDCDT, 0)
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId = chainSim.RegisterAndSetAllRolesDynamic(t, cs, accountAddrBytes, &account.nonce, issueCost, "DSFT", "DSFT", core.DynamicSFTDCDT, 0)
	supply = big.NewInt(125)
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId = chainSim.RegisterAndSetAllRoles(t, cs, accountAddrBytes, &account.nonce, issueCost, "META", "META", core.MetaDCDT, 15)
	...
	account, accountAddrBytes = createNewAccount(cs, &accountShardId)
	collectionId = chainSim.RegisterAndSetAllRolesDynamic(t, cs, accountAddrBytes, &account.nonce, issueCost, "DMETA", "DMETA", core.DynamicMetaDCDT, 10)
...
func registerTokens(
	...
	txResult := chainSim.SendTransaction(t, cs, wallet.Bytes, nonce, dcdtSafeAddress, registerCost, registerData, uint64(100000000))
	chainSim.RequireSuccessfulTransaction(t, txResult)
...
func isNft(dcdtType core.DCDTType) bool {
	return dcdtType == core.DCDTType(nonFungible) ||
		dcdtType == core.DCDTType(dynamicNFT)
}

func isMeta(dcdtType core.DCDTType) bool {
	return dcdtType == core.DCDTType(metaFungible) ||
		dcdtType == core.DCDTType(dynamicMeta)
}

func isSftOrMeta(dcdtType core.DCDTType) bool {
	return dcdtType == core.DCDTType(semiFungible) ||
		dcdtType == core.DCDTType(dynamicSFT) ||
		isMeta(dcdtType)
}

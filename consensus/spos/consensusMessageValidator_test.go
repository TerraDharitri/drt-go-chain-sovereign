...
func TestCheckMessageWithFinalInfoValidity_InvalidAggregateSignatureSize(t *testing.T) {
	...
	cnsMsg := &consensus.Message{
		PubKeysBitmap:      []byte("01"),
		AggregateSignature: []byte("0"),
	}
	err := cmv.CheckMessageWithFinalInfoValidity(cnsMsg)
...
func TestCheckConsensusMessageValidityForMessageType_MessageWithBlockBodyAndHeaderInvalid(t *testing.T) {
	...
	cnsMsg := &consensus.Message{
		MsgType:        int64(bls.MtBlockBodyAndHeader),
...
func TestCheckConsensusMessageValidityForMessageType_MessageWithInvalidSigners(t *testing.T) {
	...
	t.Run("message with header, invalid err", func(t *testing.T) {
		...
		cnsMsg := &consensus.Message{
			MsgType: int64(bls.MtInvalidSigners),
			Header:  []byte("1"),
		}
		err := cmv.CheckConsensusMessageValidityForMessageType(cnsMsg)
		assert.True(t, errors.Is(err, spos.ErrInvalidMessage))
...
func TestIsHeaderHashSizeValid_NotValid(t *testing.T) {
	t.Parallel()
	...
	result := cmv.IsHeaderHashSizeValid(cnsMsg)
	assert.False(t, result)
}

func TestIsHeaderHashSizeValid(t *testing.T) {
	t.Parallel()
...
func TestIsProcessedHeaderHashSizeValid_ShouldFail(t *testing.T) {
	t.Parallel()
	...
	processedHeaderHash := make([]byte, consensusMessageValidatorArgs.HeaderHashSize-1)
...
func createMockConsensusMessage(args spos.ArgsConsensusMessageValidator, pubKey []byte, msgType consensus.MessageType) *consensus.Message {
	msg := &consensus.Message{
		...
		HeaderHash: createDummyByteSlice(args.HeaderHashSize),
	}

	if msgType == bls.MtSignature || msgType == bls.MtBlockHeaderFinalInfo || msgType == bls.MtInvalidSigners {
		msg.ProcessedHeaderHash = createDummyByteSlice(args.HeaderHashSize)
	}

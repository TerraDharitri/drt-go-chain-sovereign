...
func TestDataCodec_SerializeTokenData(t *testing.T) {
	...
	dcdtTokenData := sovereign.DcdtTokenData{
	...
	serialized, err := abiCodec.SerializeTokenData(dcdtTokenData)
	require.Nil(t, err)
...
func TestDataCodec_DeserializeTokenData(t *testing.T) {
	...
	t.Run("token data should work", func(t *testing.T) {
		...
		expectedDcdtTokenData := &sovereign.DcdtTokenData{
			TokenType:  core.Fungible,
			Amount:     amount,
			Frozen:     false,
			Hash:       make([]byte, 0),
		...
		require.Nil(t, err)
		require.Equal(t, expectedDcdtTokenData, deserialized)
	})
}
...
func TestDataCodec_SerializeOperation(t *testing.T) {
	...
	t.Run("full operation", func(t *testing.T) {
		...
		token1 := sovereign.DcdtToken{
			Identifier: []byte("SVN-123456"),
			Nonce:      0,
			Data: sovereign.DcdtTokenData{
				TokenType:  core.Fungible,
				Amount:     amount,
				Frozen:     false,
				Hash:       []byte("hash"),
		...
		tokens := make([]sovereign.DcdtToken, 0)

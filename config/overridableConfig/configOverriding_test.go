...
func TestOverrideConfigValues(t *testing.T) {
	...
	t.Run("should work for systemSmartContractsConfig.toml", func(t *testing.T) {
		...
		configs := &config.Configs{SystemSCConfig: &config.SystemSmartContractsConfig{DCDTSystemSCConfig: config.DCDTSystemSCConfig{BaseIssuingCost: "100"}}}

		expectedNewValue := "222"
		err := OverrideConfigValues([]config.OverridableConfig{{Path: "DCDTSystemSCConfig.BaseIssuingCost", Value: expectedNewValue, File: "systemSmartContractsConfig.toml"}}, configs)
		require.NoError(t, err)
		require.Equal(t, expectedNewValue, configs.SystemSCConfig.DCDTSystemSCConfig.BaseIssuingCost)
	})

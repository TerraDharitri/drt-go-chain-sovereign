...
import (
	...
	"github.com/TerraDharitri/drt-go-chain/cmd/sovereignnode/chainSimulator/common"
	...
	proc "github.com/TerraDharitri/drt-go-chain/process"
...
func TestSovereignChainSimulator_IncomingHeader(t *testing.T) {
	...
	dcdts, _, err := nodeHandler.GetFacadeHandler().GetAllDCDTTokens(receiverWallet.Bech32, coreAPI.AccountQueryOptions{})
	require.Nil(t, err)
	require.NotNil(t, dcdts)
	require.True(t, dcdts[token] != nil)
	require.Equal(t, amountToTransfer, dcdts[token].Value.String())
}
...
func createTokenData(amountToTransfer string) sovereign.DcdtTokenData {

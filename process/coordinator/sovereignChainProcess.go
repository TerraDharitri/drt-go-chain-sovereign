...
func NewSovereignChainTransactionCoordinator(
	...
	if !ok {
		return nil, errorsDrt.ErrWrongTypeAssertion
	}
...
func (sctc *sovereignChainTransactionCoordinator) CreateMbsAndProcessCrossShardTransactionsDstMe(
	...
	if !isShardHeaderExtendedHandler {
		...
		return createMBDestMeExecutionInfo.miniBlocks, createMBDestMeExecutionInfo.numTxAdded, false, fmt.Errorf("%w from data.HeaderHandler to data.ShardHeaderExtendedHandler", errorsDrt.ErrWrongTypeAssertion)
	}

...
import (
	...
	nodeMock "github.com/TerraDharitri/drt-go-chain/node/mock"
	...
	stateAcc "github.com/TerraDharitri/drt-go-chain/state"
...
var (
	...
	sovereignNativeToken = "WREWA-bd4d79"
)
...
func getAccTokenMarshalledData(t *testing.T, tokenName []byte, account vmcommon.AccountHandler) []byte {
	...
	dcdtNFTTokenKey := computeDCDTNFTTokenKey(tokenId, 0)
...
func TestSovereignGenesisBlockCreator_initGenesisAccounts(t *testing.T) {
	...
	t.Run("cannot load dcdt sc account, should return error", func(t *testing.T) {
		_, sgbc := createSovereignGenesisBlockCreator(t)
		sgbc.arg.Accounts = &state.AccountsStub{
			LoadAccountCalled: func(container []byte) (vmcommon.AccountHandler, error) {
				if bytes.Equal(container, core.DCDTSCAddress) {
					return nil, localErr
				}
	...
	t.Run("cannot save dcdt sc account, should return error", func(t *testing.T) {
		_, sgbc := createSovereignGenesisBlockCreator(t)
		sgbc.arg.Accounts = &state.AccountsStub{
			SaveAccountCalled: func(account vmcommon.AccountHandler) error {
				if bytes.Equal(account.AddressBytes(), core.DCDTSCAddress) {
					return localErr
				}
				return nil
			},

...
def update_sovereign_config(file_path, main_chain_address, sovereign_chain_address, native_dcdt):
    ...
    updated_lines = update_key(updated_lines, "NativeDCDT", native_dcdt)

    with open(file_path, 'w') as file:
        file.writelines(updated_lines)


def update_dcdt_prefix(file_path, dcdt_prefix):
    with open(file_path, 'r') as file:
        lines = file.readlines()

    updated_lines = update_key(lines, "DCDTPrefix", dcdt_prefix)
...
def update_node_configs(config_path, dcdt_prefix, sovereign_chain_address):
    update_dcdt_prefix(config_path + "/systemSmartContractsConfig.toml", dcdt_prefix)
...
def main():
    ...
    update_sovereign_config(toml_path, main_chain_address, sovereign_chain_address, native_dcdt)

    config_path = project_path + "/cmd/node/config"
    update_node_configs(config_path, dcdt_prefix, sovereign_chain_address)

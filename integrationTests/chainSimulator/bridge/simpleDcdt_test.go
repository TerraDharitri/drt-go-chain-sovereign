...
const (
	simpleDcdtTestWasmPath = "testdata/simple-dcdt-test.wasm"
...
func TestSovereignChainSimulator_CreateNftWithManyQuantityShouldFail(t *testing.T) {
	...
	expectedError := "invalid arguments to process built-in function, invalid quantity for dcdt type"
	...
	executeSimpleDcdtOperationWithError(t, txData, expectedFullError)
...
func TestSovereignChainSimulator_CreateNftWithInvalidTypeShouldFail(t *testing.T) {
	...
	expectedError := "invalid arguments to process built-in function, invalid dcdt type"
	...
	expectedFullError = fmt.Sprintf("%s %d (%s)", expectedError, core.DCDTType(8), core.DCDTType(8).String())
	txData = createNftCreateArgs("da-NFT-beb731", uint64(7), big.NewInt(1), core.DCDTType(8), creatorAddress)
	executeSimpleDcdtOperationWithError(t, txData, expectedFullError)
}

func executeSimpleDcdtOperationWithError(
	t *testing.T,
	txData string,
	expectedErr string,
) {
	...
	contractAddress := chainSim.DeployContract(t, cs, initialAddrBytes, &nonce, systemContractDeploy, "", simpleDcdtTestWasmPath)
...
func TestChainSimulator_CreateAndBurnAllDcdtTypes(t *testing.T) {
	if testing.Short() {
		t.Skip("this is not a short test")
	}
	...
	tokens := createAllDcdtTypes(t, cs, initialAddrBytes, contractAddress, &nonce)
	for _, token := range tokens {
		checkMetaDataInAccounts(t, cs, token, initialAddress, token.Amount)
	}

	burnAllDcdtTypes(t, cs, initialAddrBytes, contractAddress, &nonce, tokens)
	for _, token := range tokens {
		checkMetaDataInAccounts(t, cs, token, initialAddress, big.NewInt(0))
	}
}

func createAllDcdtTypes(
	t *testing.T,
...
func createNftCreateArgs(
	...
	return "nft_create" +
		...
		"@" + getTokenNonce(nonce) +
		"@" + hex.EncodeToString(amount.Bytes()) +
		"@" + fmt.Sprintf("%02x", uint32(dcdtType)) +
		"@" + creator
}

func burnAllDcdtTypes(
	t *testing.T,
	cs chainSim.ChainSimulator,
	sender, contract []byte,
	nonce *uint64,
	...
	for _, token := range tokens {
		if token.Type == core.Fungible {
			chainSim.TransferDCDT(t, cs, sender, contract, nonce, token.Identifier, token.Amount, []byte(burnEndpoint))
		} else {
			chainSim.TransferDCDTNFT(t, cs, sender, contract, nonce, token.Identifier, token.Nonce, token.Amount, []byte(burnEndpoint))
		}
	}
...
func TestChainSimulator_CreateTokenAndNFTCollectionSameIdentifierAndMakeTransactions(t *testing.T) {
	...
	initialAddress := "drt1l6xt0rqlyzw56a3k8xwwshq2dcjwy3q9cppucvqsmdyw8r98dz3sq9c49p"
	initialAddrBytes, _ := nodeHandler.GetCoreComponents().AddressPubKeyConverter().Decode(initialAddress)
	bobAddress := "drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c"
	...
	chainSim.TransferDCDT(t, cs, initialAddrBytes, bobAddrBytes, &nonce, tokenIdentifier, tokenAmountToSend)
	...
	chainSim.TransferDCDTNFT(t, cs, initialAddrBytes, bobAddrBytes, &nonce, nftCollection, nftNonce, nftAmount)
	...
	chainSim.TransferDCDTNFT(t, cs, bobAddrBytes, initialAddrBytes, &bobNonce, nftCollection, nftNonce, nftAmount)
	...
	chainSim.TransferDCDT(t, cs, initialAddrBytes, contractAddress, &nonce, tokenIdentifier, burnAmount, []byte(burnEndpoint))
	token.Amount = big.NewInt(0).Sub(token.Amount, burnAmount)
	checkTokenDataInAccount(t, cs, token, initialAddress, token.Amount)

	// burn NFT from initial address
	chainSim.TransferDCDTNFT(t, cs, initialAddrBytes, contractAddress, &nonce, nftCollection, nftNonce, nftAmount, []byte(burnEndpoint))
	checkTokenDataInAccount(t, cs, nft, initialAddress, big.NewInt(0))

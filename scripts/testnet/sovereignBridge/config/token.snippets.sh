issueToken() {
    ...
    drtpy contract call ${DCDT_SYSTEM_SC_ADDRESS} \
        --pem=${WALLET} \
        --proxy=${PROXY} \
        --chain=${CHAIN_ID} \
        --gas-limit=60000000 \
    ...
    local HEX_TOKEN_IDENTIFIER=$(drtpy data parse --file=${OUTFILE}  --expression="data['transactionOnNetwork']['logs']['events'][2]['topics'][0]")
...
depositTokenInSC() {
    ...
    echo "Depositing token in DCDT Safe contract on main chain..."
    ...
    drtpy contract call ${WALLET_ADDRESS} \
        --pem=${WALLET} \
        --proxy=${PROXY} \
        --chain=${CHAIN_ID} \
        --gas-limit=20000000 \
...
registerSovereignToken() {
    echo "Registering Sovereign token in DCDT Safe contract on main chain..."
...
setLocalBurnRoleSovereign() {
    echo "Setting local burn role for DCDT Safe contract on sovereign chain..."
    checkVariables DEPOSIT_TOKEN_IDENTIFIER_SOVEREIGN DCDT_SAFE_ADDRESS_SOVEREIGN || return
...
depositTokenInSCSovereign() {
    echo "Depositing token in DCDT Safe contract on sovereign chain..."

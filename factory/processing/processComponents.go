...
func (pcf *processComponentsFactory) Create() (*processComponents, error) {
	...
	crossChainTokenChecker, err := vmcommonBuiltInFunctions.NewCrossChainTokenChecker([]byte(pcf.systemSCConfig.DCDTSystemSCConfig.DCDTPrefix), mapWhiteListedCrossChainAddresses)
	if err != nil {
		return nil, err
	}
...
func checkProcessComponentsArgs(args ProcessComponentsFactoryArgs) error {
	...
	if check.IfNil(args.RunTypeComponents) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilRunTypeComponents)
	}
	if check.IfNil(args.RunTypeComponents.BlockChainHookHandlerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilBlockChainHookHandlerCreator)
	}
	if check.IfNil(args.RunTypeComponents.BlockProcessorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilBlockProcessorCreator)
	}
	if check.IfNil(args.RunTypeComponents.BlockTrackerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilBlockTrackerCreator)
	}
	if check.IfNil(args.RunTypeComponents.BootstrapperFromStorageCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilBootstrapperFromStorageCreator)
	}
	if check.IfNil(args.RunTypeComponents.BootstrapperCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilBootstrapperCreator)
	}
	if check.IfNil(args.RunTypeComponents.EpochStartBootstrapperCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilEpochStartBootstrapperCreator)
	}
	if check.IfNil(args.RunTypeComponents.ForkDetectorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilForkDetectorCreator)
	}
	if check.IfNil(args.RunTypeComponents.HeaderValidatorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilHeaderValidatorCreator)
	}
	if check.IfNil(args.RunTypeComponents.RequestHandlerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilRequestHandlerCreator)
	}
	if check.IfNil(args.RunTypeComponents.ScheduledTxsExecutionCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilScheduledTxsExecutionCreator)
	}
	if check.IfNil(args.RunTypeComponents.TransactionCoordinatorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilTransactionCoordinatorCreator)
	}
	if check.IfNil(args.RunTypeComponents.ValidatorStatisticsProcessorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilValidatorStatisticsProcessorCreator)
	}
	if check.IfNil(args.RunTypeComponents.AdditionalStorageServiceCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilAdditionalStorageServiceCreator)
	}
	if check.IfNil(args.RunTypeComponents.SCResultsPreProcessorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilSCResultsPreProcessorCreator)
	}
	if check.IfNil(args.RunTypeComponents.SCProcessorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilSCProcessorCreator)
	}
	// TODO make sure consensus model is correct on higher level - task DRT-15366
	if args.RunTypeComponents.ConsensusModel() == consensus.ConsensusModelInvalid {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrInvalidConsensusModel)
	}
	if check.IfNil(args.RunTypeComponents.VmContainerMetaFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilVmContainerMetaFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.VmContainerShardFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilVmContainerShardFactoryCreator)
	}
	...
	if check.IfNil(args.RunTypeComponents.AccountsCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilAccountsCreator)
	}
	if check.IfNil(args.RunTypeComponents.OutGoingOperationsPoolHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilOutGoingOperationsPool)
	}
	if check.IfNil(args.RunTypeComponents.DataCodecHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilDataCodec)
	}
	if check.IfNil(args.RunTypeComponents.TopicsCheckerHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilTopicsChecker)
	}
	if check.IfNil(args.RunTypeComponents.ShardCoordinatorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilShardCoordinatorFactory)
	}
	if check.IfNil(args.RunTypeComponents.RequestersContainerFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilRequesterContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.InterceptorsContainerFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilInterceptorsContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.ShardResolversContainerFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilShardResolversContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.TxPreProcessorCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilTxPreProcessorCreator)
	}
	if check.IfNil(args.RunTypeComponents.ExtraHeaderSigVerifierHolder()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilExtraHeaderSigVerifierHolder)
	}
	if check.IfNil(args.RunTypeComponents.GenesisBlockCreatorFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilGenesisBlockFactory)
	}
	if check.IfNil(args.RunTypeComponents.GenesisMetaBlockCheckerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilGenesisMetaBlockChecker)
	}
	if check.IfNil(args.RunTypeComponents.EpochStartTriggerFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilEpochStartTriggerFactory)
	}
	if check.IfNil(args.RunTypeComponents.StakingToPeerFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilStakingToPeerFactory)
	}
	if check.IfNil(args.RunTypeComponents.ValidatorInfoCreatorFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilValidatorInfoCreatorFactory)
	}
	if check.IfNil(args.RunTypeComponents.ApiProcessorCompsCreatorHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilAPIProcessorCompsCreator)
	}
	if check.IfNil(args.RunTypeComponents.EndOfEpochEconomicsFactoryHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilEndOfEpochEconomicsFactory)
	}
	if check.IfNil(args.RunTypeComponents.RewardsCreatorFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilRewardsFactory)
	}
	if check.IfNil(args.RunTypeComponents.SystemSCProcessorFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilSysSCFactory)
	}
	if check.IfNil(args.RunTypeComponents.PreProcessorsContainerFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilPreProcessorsContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.DataRetrieverContainersSetter()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilDataRetrieverContainersSetter)
	}
	if check.IfNil(args.RunTypeComponents.ExportHandlerFactoryCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilExportHandlerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.OutportDataProviderFactory()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilOutportDataProviderFactory)
	}
	if check.IfNil(args.IncomingHeaderSubscriber) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsDrt.ErrNilIncomingHeaderSubscriber)
	}
